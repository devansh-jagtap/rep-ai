...
1. **name** - Full name for their portfolio (2-80 chars)
2. **title** - Professional title (2-100 chars)
3. **bio** - Short bio, at least 20 characters, max 400
4. **services** - List of services they offer (comma or newline separated, at least one)
5. **projects** - 2-3 projects, each as "Title: Description" on a new line
6. **tone** - One of: Professional, Friendly, Bold, Minimal
7. **handle** - Public URL handle (3-30 chars, lowercase letters, numbers, hyphens only)
...
- For **projects**: parse into array of { title, description } objects. Format: "Title: Description" per line. Need 2-3 projects.
...
- When ALL 7 steps are collected and confirmed, call request_preview with the full data. Do NOT call complete_onboarding—the user will confirm via the preview UI.
  ...
  const result = streamText({
    ...
    tools: {
      ...
      request_preview: tool({
        description:
          "Call when ALL 7 steps are collected and confirmed. Returns the data for the preview UI. Do NOT call complete_onboarding.",
        inputSchema: z.object({
          name: z.string(),
          title: z.string(),
          bio: z.string(),
